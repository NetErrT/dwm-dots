...
static const char *launchercmd[] = {"dmenu_run", "-nb", theme[1], "-nf",
                                    ...
                                    theme[0],   NULL};
static const char *terminalcmd[] = {"xterm", NULL};
...
static const Key keys[] = {
    ...
    {0, XF86XK_AudioMicMute, spawn, SHCMD("amixer sset Speaker toggle")},
		{0, XK_Print, spawn, SHCMD("scrot \"$(xdg-user-dir PICTURES)/screenshots/$(date +'%Y.%m.%d_%H.%M.%S').png\" -q 100")},
		{0 | ShiftMask, XK_Print, spawn, SHCMD("scrot -s \"$(xdg-user-dir PICTURES)/screenshots/$(date +'%Y.%m.%d_%H.%M.%S').png\" -q 100")},
};
